...
st.set_page_config(page_title="Inventory Management System - Final", layout="wide")

# Paths and demo file locations (expect a data/ folder in the repo)
...
# Free rule-based reviewer
def free_review_suggester(reviews):
    texts = [str(t).lower() for t in reviews.astype(str).tolist() if str(t).strip()]
    if not texts:
        return ["No reviews available."]
    neg_kw = ["broken","defect","overheat","noise","noisy","delay","late","refund","return","compat","crash","slow"]
    pos_kw = ["good","great","excellent","works","stable","fast","recommend","love","nice","perfect"]
    neg_count = sum(any(k in t for k in neg_kw) for t in texts)
    pos_count = sum(any(k in t for k in pos_kw) for t in texts)
    suggestions = []
    suggestions.append(f"Analyzed {len(texts)} reviews — positive-like: {pos_count}, negative-like: {neg_count}")
    # find complaint snippets
    snippets = {}
    for t in texts:
        for k in neg_kw:
            if k in t:
                start = max(0, t.find(k)-30)
                snippet = t[start: t.find(k)+30].strip()
                snippets[snippet] = snippets.get(snippet, 0) + 1
    top = sorted(snippets.items(), key=lambda x: x[1], reverse=True)[:5]
    if top:
        suggestions.append("Frequent complaint snippets:")
        for s,c in top:
            suggestions.append(f"- ({c}) {s}")
    # heuristics
    if neg_count > pos_count:
        suggestions.append("Investigate quality issues: negative reports exceed positives.")
    if any("overheat" in t for t in texts):
        suggestions.append("Check thermal design and provide cooling guidance in docs.")
    if any("compat" in t for t in texts):
        suggestions.append("Publish compatibility matrix and firmware instructions.")
    if not suggestions:
        suggestions.append("No clear improvement detected; collect more feedback.")
    return suggestions
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("Data")
    sales_up = st.file_uploader("Sales CSV", type=["csv"], key="su")
    stock_up = st.file_uploader("Stock CSV", type=["csv"], key="st")
    reviews_up = st.file_uploader("Reviews CSV", type=["csv"], key="rv")
    cols = st.columns(3)
    load_demo = cols[0].button("Load demo")
    use_last = cols[1].checkbox("Use last (local only)")
    st.markdown("---")
    st.info("For Streamlit Cloud: include demo files in data/ in the repo and click Load demo.")

# persist uploads locally for convenience
...
if sales_file is None:
    st.info("Upload sales CSV or include demo files in data/ then click Load demo.")
    st.stop()

sales_df, err = safe_read(sales_file)
if sales_df is None:
    st.error(f"Error reading sales: {err}")
    st.stop()
try:
    sales_df = normalize_sales_df(sales_df)
except Exception as e:
    st.error(f"Error normalizing sales: {e}")
    st.stop()
...
# --- Inventory ---
    ...
    if stock_df is None:
        st.info("Upload stock CSV to view stock and recommendations.")
    else:
    ...
    for p in sel:
        ...
        if use_prophet and not ts.empty:
            try:
                ...
                fc_future = fc[fc['ds'] > last_date].head(int(horizon))
                forecast = float(fc_future['yhat'].sum())
            except Exception:
                forecast = float(recent_avg * int(horizon))
        else:
            forecast = float(recent_avg * int(horizon))
        cur_stock = None
        ...
        safety = safety_days * (recent_avg if recent_avg>0 else 1)
        ...
        recs.append({
            ...
            f"Forecast_{horizon}d": round(forecast,2),
...
# --- Reports ---
    ...
    if ts.empty:
        ...
        if use_prophet:
            ...
            future_dates = pd.date_range(start=ts['Date'].max()+pd.Timedelta(days=1), periods=horizon, freq='D')
