...
PROFILE_PATH = os.path.join(PERSIST_DIR, "ims_profile.json")
...
def normalize_sales_df(df):
    df = df.copy()
    # find date, product, sales columns
    for c in df.columns:
        if "date" in c.lower():
...
with st.sidebar:
    ...
    remove_demo = cols[2].button("Remove demo selection")
    clear_last = cols[3].button("Clear last persisted files")
    st.markdown("---")
    st.header("Forecast")
    ...
    st.markdown("---")
    st.caption("Tip: for Streamlit Cloud include demo files in data/ and click Load demo.")

# handle clear/remove actions
if clear_last:
    for fn in ["last_sales.csv", "last_stock.csv", "last_reviews.csv"]:
        p = os.path.join(PERSIST_DIR, fn)
        try:
            if os.path.exists(p): os.remove(p)
        except Exception:
            pass
    st.sidebar.success("Cleared last persisted files (local runtime only).")
if remove_demo:
    load_demo = False
    st.sidebar.success("Demo selection cleared.")
...
def choose_file(uploaded, demo_path, persist_name):
    if uploaded is not None: return uploaded
...
if sales_file is None:
    st.info("Please upload Sales CSV, or add demo files and click Load demo.")
    st.stop()
...
if page == "Dashboard":
    ...
    if st.button("Save profile"):
        p = {"name":name,"company":company,"email":email}
        ...
        try:
            ...
            st.warning("Profile saved to session only.")
